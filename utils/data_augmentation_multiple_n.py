...
def data_augmentation_methods(mats, direct, quitar_slices, SEED, x_train_shape0, x_train, x_train_noisy, flipleft, flipupdown, zoom):
    ...
    if flipleft == 'si': 
        ...
        images_noise = list(x_train_noisy)
        for i in list(range(len(images_noise))):
            flipleft = np.flip(x_train_noisy[i,:,:])
    ...
    if flipupdown == 'si': 
        ...
        for i in list(range(len(x_train_noisy))):
            flipupdown = np.flipud(x_train_noisy[i,:,:])
    ...
    if zoom == 'si':
        ...
        for i in list(range(len(x_train_noisy))):
            ...
            #Bucle zoom noise
            zoom = resize_and_crop(x_train_noisy[i,:,:], resize_factor)
